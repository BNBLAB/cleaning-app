...
export async function getServerSideProps({ query }) {
  ...
  // 空欄の日に「ゲスト滞在中」を表示するため、表示中の週と重なる予約をまとめて取得
  const roomIdToProperty = {};
  (rooms ?? []).forEach((r) => {
    roomIdToProperty[r.id] = { propertyId: r.property_id, roomName: r.name };
  });
  const allRoomIds = Object.keys(roomIdToProperty);

  let stays = [];
  if (allRoomIds.length > 0) {
    const { data: overlapping } = await supabase
      .from("bookings")
      .select("room_id, check_in, check_out, guest_name, num_adult, num_child, status")
      .in("room_id", allRoomIds)
      .neq("status", "cancelled")
      .lte("check_in", weekEndISO)
      .gt("check_out", weekStartISO);

    (overlapping ?? []).forEach((b) => {
      const info = roomIdToProperty[b.room_id];
      if (!info) return;
      let d = new Date(b.check_in + "T00:00:00Z");
      const end = new Date(b.check_out + "T00:00:00Z");
      while (d < end) {
        const iso = d.toISOString().slice(0, 10);
        if (iso >= weekStartISO && iso <= weekEndISO && iso !== b.check_out) {
          stays.push({
            propertyId: info.propertyId,
            roomName: info.roomName,
            date: iso,
            guestName: b.guest_name,
            guests: (b.num_adult ?? 0) + (b.num_child ?? 0),
            nights: Math.round((end - new Date(b.check_in + "T00:00:00Z")) / (1000 * 60 * 60 * 24)),
            checkIn: b.check_in,
    ...
    // チェックアウト当日（＝清掃タスクとして既に表示される日）は除外する
    const checkoutKeySet = new Set(tasks.map((t) => `${t.roomName}|${t.date}`));
    stays = stays.filter((s) => !checkoutKeySet.has(`${s.roomName}|${s.date}`));
  }
...
export default function Home({ tasks, properties, specialAssignees, today, weekStart, showHidden, errorMessage, assigneeOptions, shiftsInWeek, roomOptions, stays }) {
